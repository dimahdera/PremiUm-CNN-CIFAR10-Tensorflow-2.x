...
def main_function(input_dim=32, n_channel=3, num_kernels=[32, 32, 32, 32, 64, 64, 64, 128, 128, 128],
                  ...
                  batch_size=50, epochs=500, lr=0.001, lr_end = 0.0001, kl_factor=0.00001,
                  Random_noise=False, gaussain_noise_std=0, Adversarial_noise=True, epsilon=0.005,
                  adversary_target_cls=3, Targeted=False,
                  Training=False, continue_training=False, saved_model_epochs=250):
    ...
    def create_adversarial_pattern(input_image, input_label):
          ...
          return signed_grad  
    if Training: 
       # wandb.init(entity = "dimah", project="VDP_CNN_Cifar10_11layers_epochs_{}_lr_{}".format(epochs, lr)) 
...
#                       }) 
    ...
    # -------------------------Testing-----------------------------
        ...
        for step, (x, y) in enumerate(val_dataset):
            ...
            if step % 1000 == 0:
                print("Total running accuracy so far: %.4f" % np.mean(np.amax(acc_test) ) )
            test_no_steps += 1
