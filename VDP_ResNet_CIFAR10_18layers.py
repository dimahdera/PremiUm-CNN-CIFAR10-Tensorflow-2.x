...
class VDP_first_Conv(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.w_mu = self.add_weight(name='w_mu',
                                    ...
                                    regularizer=tf.keras.regularizers.l1_l2(l1=tau, l2=tau),  #l1_l2(l1=tau, l2=tau)
                                    trainable=True,
                                    )
...
class VDP_intermediate_Conv(keras.layers.Layer):
    def __init__(self, kernel_size=5, kernel_num=16, kernel_stride=2, padding="VALID"):
...
class VDP_Flatten_and_FC(keras.layers.Layer):   
    ...
    def build(self, input_shape):
        ...
        self.w_mu = self.add_weight(name = 'w_mu', shape=(input_shape[1]*input_shape[2]*input_shape[-1], self.units),
            initializer=tf.random_normal_initializer( mean=0.0, stddev=0.05, seed=None), regularizer=tf.keras.regularizers.l1_l2(l1=tau, l2=tau),#l1_l2(l1=tau, l2=tau), 
            trainable=True,
        )
        self.w_sigma = self.add_weight(name = 'w_sigma',
            shape=(self.units,),
...
class VDPGAP(keras.layers.Layer):
    def __init__(self):
        super(VDPGAP, self).__init__()
    def call(self, mu_in, Sigma_in):
        n = mu_in.shape[1]*mu_in.shape[1]
        mu_out = tf.reduce_mean(mu_in, axis=[1, 2])
        sigma_out = (1/n )*tf.reduce_mean(Sigma_in, axis=[1, 2])
        Sigma_out = tf.linalg.diag(sigma_out)
        return mu_out, Sigma_out
...
class VDPResidualBlock(keras.layers.Layer):
    def __init__(self, kernel_size=3, kernel_num=32, kernel_stride=1, padding="SAME", var_epsilon=1e-4):
        super(VDPResidualBlock, self).__init__()
        self.kernel_size = kernel_size
        self.kernel_num = kernel_num
        self.kernel_stride = kernel_stride
        self.padding = padding
    ...
    def build(self, input_shape):        
        tau = -1. #/ (self.kernel_size * self.kernel_size * input_shape[-1])
        self.w_mu1 = self.add_weight(name='w_mu1',
                                    shape=(self.kernel_size, self.kernel_size, input_shape[-1], self.kernel_num),
                                    initializer=tf.random_normal_initializer(mean=0.0, stddev=0.05, seed=None),
                                    regularizer=tf.keras.regularizers.l1_l2(l1=tau, l2=tau),  #l1_l2(l1=tau, l2=tau)
                                    trainable=True,
        ...
        self.w_mu2 = self.add_weight(name='w_mu2',
                                    shape=(self.kernel_size, self.kernel_size, input_shape[-1], self.kernel_num),
                                    initializer=tf.random_normal_initializer(mean=0.0, stddev=0.05, seed=None),
                                    regularizer=tf.keras.regularizers.l1_l2(l1=tau, l2=tau),  #l1_l2(l1=tau, l2=tau)
                                    trainable=True,
    ...
    def call(self, mu_in, Sigma_in):
        ...
        conv_out_1 = tf.nn.conv2d(mu_in, self.w_mu1, strides=[1, self.kernel_stride, self.kernel_stride, 1],
                              padding=self.padding, data_format='NHWC')
        mean, variance = tf.nn.moments(conv_out_1, [0, 1, 2])
        norm_out_1 = tf.nn.batch_normalization(conv_out_1, mean, variance, offset=None, scale=None,
                                         variance_epsilon=self.var_epsilon) 
        relu_out = tf.nn.relu(norm_out_1)
        conv_out_2 = tf.nn.conv2d(relu_out, self.w_mu2, strides=[1, self.kernel_stride, self.kernel_stride, 1],
                            padding=self.padding, data_format='NHWC')
        mean, variance = tf.nn.moments(conv_out_2, [0, 1, 2])
        norm_out_2 = tf.nn.batch_normalization(conv_out_2, mean, variance, offset=None, scale=None,
                                         variance_epsilon=self.var_epsilon)     
        mu_out =  norm_out_2 + mu_in   
        Sigma_out = tf.linalg.set_diag(Sigma_in, tf.abs(tf.linalg.diag_part(Sigma_in)))  
        return mu_out, Sigma_out        
                        
class Density_prop_ResNet(tf.keras.Model):
    def __init__(self, kernel_size, num_kernel, units, drop_prop=0.2,  var_epsilon=1e-4, name=None):#
        super(Density_prop_ResNet, self).__init__()
        self.kernel_size = kernel_size
        ...
        self.conv_1 = VDP_first_Conv(kernel_size=self.kernel_size, kernel_num=self.num_kernel[0], padding='VALID')
        self.res_1 = VDPResidualBlock(kernel_size=self.kernel_size, kernel_num=self.num_kernel[1],var_epsilon=self.var_epsilon)
        self.res_2 = VDPResidualBlock(kernel_size=self.kernel_size, kernel_num=self.num_kernel[2],var_epsilon=self.var_epsilon)
        self.res_3 = VDPResidualBlock(kernel_size=self.kernel_size, kernel_num=self.num_kernel[3],var_epsilon=self.var_epsilon)        
        self.conv_2 = VDP_intermediate_Conv(kernel_size=self.kernel_size, kernel_num=self.num_kernel[4], kernel_stride=2, padding="VALID")
        self.res_4 = VDPResidualBlock(kernel_size=self.kernel_size, kernel_num=self.num_kernel[5],var_epsilon=self.var_epsilon)
        self.res_5 = VDPResidualBlock(kernel_size=self.kernel_size, kernel_num=self.num_kernel[6],var_epsilon=self.var_epsilon)
        #self.res_6 = VDPResidualBlock(kernel_size=self.kernel_size, kernel_num=self.num_kernel[7],var_epsilon=self.var_epsilon)   
        self.conv_3 = VDP_intermediate_Conv(kernel_size=self.kernel_size, kernel_num=self.num_kernel[7], kernel_stride=2, padding="VALID")
        self.res_7 = VDPResidualBlock(kernel_size=self.kernel_size, kernel_num=self.num_kernel[8],var_epsilon=self.var_epsilon)
        self.res_8 = VDPResidualBlock(kernel_size=self.kernel_size, kernel_num=self.num_kernel[9],var_epsilon=self.var_epsilon)
        #self.res_9 = VDPResidualBlock(kernel_size=self.kernel_size, kernel_num=self.num_kernel[11],var_epsilon=self.var_epsilon)       

        self.relu_1 = VDPReLU()
        self.gap_1 = VDPGAP()
        self.dropout_1 = VDPDropout(self.drop_prop)        
        self.batch_norm = VDPBatch_Normalization(self.var_epsilon)
        self.fc_1 = VDP_FC(self.units) #VDP_Flatten_and_FC(self.units)
        self.mysoftma = mysoftmax()

    def call(self, inputs, training=True):
        mu, sigma = self.conv_1(inputs)          
        mu, sigma = self.batch_norm(mu, sigma)
        mu, sigma = self.relu_1(mu, sigma)
        #####################################
        mu, sigma = self.res_1(mu, sigma)
        mu, sigma = self.relu_1(mu, sigma)
        mu, sigma = self.res_2(mu, sigma)
        mu, sigma = self.relu_1(mu, sigma)
        mu, sigma = self.res_3(mu, sigma)
        mu, sigma = self.relu_1(mu, sigma)
        mu, sigma = self.dropout_1(mu, sigma)
        #####################################
        mu, sigma = self.conv_2(mu, sigma)         
        mu, sigma = self.batch_norm(mu, sigma)
        mu, sigma = self.relu_1(mu, sigma)
        #####################################
        mu, sigma = self.res_4(mu, sigma)
        mu, sigma = self.relu_1(mu, sigma)
        mu, sigma = self.res_5(mu, sigma)
        mu, sigma = self.relu_1(mu, sigma)
#        mu, sigma = self.res_6(mu, sigma)
#        mu, sigma = self.relu_1(mu, sigma)
        mu, sigma = self.dropout_1(mu, sigma)
        #####################################       
        ...
        mu, sigma = self.relu_1(mu, sigma)
        #####################################  
        mu, sigma = self.res_7(mu, sigma)
        mu, sigma = self.relu_1(mu, sigma)
        mu, sigma = self.res_8(mu, sigma)
        mu, sigma = self.relu_1(mu, sigma)
#        mu, sigma = self.res_9(mu, sigma)
#        mu, sigma = self.relu_1(mu, sigma)
        mu, sigma = self.dropout_1(mu, sigma)
        #####################################
        mu, sigma = self.gap_1(mu, sigma )             
...
def main_function(input_dim=32, n_channel=3, num_kernels=[16,16,16, 16, 32, 32,32, 64,64, 64],
                  kernels_size=3,  class_num=10,   batch_size=50, epochs=3, lr=0.0002, lr_end = 0.0001, kl_factor=0.001,
                  Random_noise=False, Adversarial_noise=False, HCV=0.1,  Black_box_attack=True, PGDBlack_box_attack=True, 
                  adversary_target_cls=3, Targeted=True, PGD_Adversarial_noise=False, stepSize=1, maxAdvStep=40,
                  Training=False, Testing = False, continue_training=False, saved_model_epochs=12):
    PATH = './ResNet_saved_models/VDP_resnet_epoch_{}/'.format(epochs)
    (x_train, y_train), (x_test, y_test) = cifar10.load_data() 
    x_train, x_test = x_train / 255.0, x_test / 255.0 
    x_train = x_train.astype('float32')
    x_test = x_test.astype('float32')

    cnn_model = Density_prop_ResNet(kernel_size=kernels_size, num_kernel=num_kernels, units=class_num,
                                 name='vdp_resnet')
    ...
    def train_on_batch(x, y):
        ...
        gradients = [(tf.where(tf.math.is_nan(grad), tf.constant(1.0e-6, shape=grad.shape), grad)) for grad in gradients]
        gradients = [(tf.where(tf.math.is_inf(grad), tf.constant(1.0e-6, shape=grad.shape), grad)) for grad in gradients]
    ...
    def validation_on_batch(x, y):                     
        ...
        vloss = nll_gaussian(y, mu_out,  tf.clip_by_value(t=sigma, clip_value_min=tf.constant(-1e+5),
                                           clip_value_max=tf.constant(1e+5)), class_num , batch_size)                                           
        regularization_loss=tf.math.add_n(cnn_model.losses)
    ...
    def create_adversarial_pattern(input_image, input_label):
          with tf.GradientTape() as tape:
            ...
            loss_final = nll_gaussian(input_label, prediction,  tf.clip_by_value(t=sigma, clip_value_min=tf.constant(-1e+5),
                                   clip_value_max=tf.constant(1e+5)), class_num , batch_size)                         
            loss = 0.5 * loss_final 
          ...
          return signed_grad 
    num_folds = 10  
    valid_size = 60000/  num_folds
    train_size = 60000 - valid_size        
    if Training: 
        wandb.init(entity = "dimah", project="VDP_ResNET_Cifar10_18layers_epochs_{}_lr_{}_kfold".format(epochs, lr)) 
        if continue_training:
            saved_model_path = './ResNet_saved_models/VDP_resnet_epoch_{}/'.format(saved_model_epochs)
        ...
        for train, test in kfold.split(inputs, targets):    
            ...
            one_hot_y_train = tf.one_hot(np.squeeze(targets[train]).astype(np.float32), depth=class_num)
            one_hot_y_test = tf.one_hot(np.squeeze(targets[test]).astype(np.float32), depth=class_num)
            tr_dataset = tf.data.Dataset.from_tensor_slices((inputs[train], one_hot_y_train)).batch(batch_size)
            val_dataset = tf.data.Dataset.from_tensor_slices((inputs[test], one_hot_y_test)).batch(batch_size) 
            ...
            for epoch in range(epochs): 
                ...
                va_no_steps = 0
                # -------------Training--------------------
                acc_training = np.zeros(int(targets[train].shape[0] / (batch_size)))
                err_training = np.zeros(int(targets[train].shape[0] / (batch_size)))
                for step, (x, y) in enumerate(tr_dataset):
                    update_progress(step / int(targets[train].shape[0] / (batch_size)))
                    ...
                    if step % 100 == 0:
                        ...
                        print('\n Matrix Norm', np.mean(sigma))    # tf.norm(sigma, ord='fro', axis=[-2, -1])))
                ...
                # ---------------Validation----------------------
                acc_validation = np.zeros(int(targets[test].shape[0] / (batch_size)))
                err_validation = np.zeros(int(targets[test].shape[0] / (batch_size)))            
                for step, (x, y) in enumerate(val_dataset):
                    update_progress(step / int(targets[test].shape[0] / (batch_size)))
                ...
                # -----------------End Training--------------------------
            acc_per_fold.append(np.mean(valid_acc[epoch]))
            loss_per_fold.append(np.mean(valid_err[epoch]))  
            print('------------------------------------')             
            print('Validation Acc per K fold', np.mean( acc_per_fold ))            
            print('Validation error per K fold', np.mean( loss_per_fold ) ) 
            print('------------------------------------') 
            fold_no = fold_no + 1     
            cnn_model.save_weights(PATH + 'vdp_cnn_model')
            if (epochs > 1):
                ...
                plt.ylim(0, 1.1)
                plt.title("Density Propagation ResNet on CIFAR-10 Data")
                plt.xlabel("Epochs")
                plt.ylabel("Accuracy")
                plt.legend(loc='lower right')
                plt.savefig(PATH + 'VDP_ResNet_on_CIFAR10_Data_acc.png')
                plt.close(fig)
    
                fig = plt.figure(figsize=(15, 7))
                plt.plot(train_err, 'b', label='Training error')
                plt.plot(valid_err, 'r', label='Validation error')
                # plt.ylim(0, 1.1)
                plt.title("Density Propagation ResNet on CIFAR-10 Data")
                plt.xlabel("Epochs")
                plt.ylabel("Error")
                plt.legend(loc='upper right')
                plt.savefig(PATH + 'VDP_ResNet_on_CIFAR10_Data_error.png')
                plt.close(fig)
            ...
            if Training:
                ...
                if (epochs == 1):
                    ...
                    textfile.write("\n Validation Accuracy : " + str(np.mean(acc_per_fold)))                
                    textfile.write("\n Training error : " + str(np.mean(train_err[epoch])))                
                    textfile.write("\n Validation error : " + str(np.mean(loss_per_fold)))
            textfile.write("\n---------------------------------")
            textfile.write("\n---------------------------------")
            textfile.close()
    # -------------------------Testing-----------------------------
    if (Testing):
        ...
        true_x = np.zeros([int(valid_size / (batch_size)), batch_size, input_dim, input_dim, n_channel])
    ...
    if (Adversarial_noise):
        ...
        test_no_steps = 0        
        true_x = np.zeros([int(valid_size / (batch_size)), batch_size, input_dim, input_dim, n_channel])
        adv_perturbations = np.zeros([int(valid_size / (batch_size)), batch_size, input_dim, input_dim,n_channel])
        true_y = np.zeros([int(valid_size / (batch_size)), batch_size, class_num])
        mu_out_ = np.zeros([int(valid_size / (batch_size)), batch_size, class_num])
        sigma_ = np.zeros([int(valid_size / (batch_size)), batch_size, class_num, class_num])
        acc_test = np.zeros(int(valid_size/ (batch_size)))
...
#               adv_x1 = tf.clip_by_value(adv_perturbations[test_no_steps, :, :, :], - epsilon, epsilon)
#               adv_x = tf.clip_by_value(adv_x1+x, 0.0, 1.0)
#                 
#               adv_x = x + epsilon * adv_perturbations[test_no_steps, :, :, :]
#               adv_x = tf.clip_by_value(adv_x, 0.0, 1.0)
         #      adv_x = adv_perturbations[test_no_steps, :, :, :]
               mu_out, sigma   = test_on_batch(adv_x, y)            
               mu_out_[test_no_steps, :, :] = mu_out
               sigma_[test_no_steps, :, :, :] = sigma
               corr = tf.equal(tf.math.argmax(mu_out, axis=-1), tf.math.argmax(y, axis=-1))
